...
class PersonCenter extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {};
  }
  ...
  async componentDidMount() {
    ...
    console.log("loginnameloginname", loginname);
    let userMessage = await queryUser(loginname);
    if (userMessage.status === 200 && userMessage.data.success) {
      console.log("userMessage.data.data", userMessage.data.data);
      const {
        loginname, //用户名
        avatar_url, //头像
        score, //积分
              ...
              <View style={styles.header}>
                <View style={styles.inner}>
                  ...
                  <View style={styles.col}>
                    <Text style={[styles.span, styles.name]}>
                      {user.loginname}
                    </Text>
